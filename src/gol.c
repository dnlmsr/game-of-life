...
void initUniverse(bool universe[MAX_X][MAX_Y], char filePath[]) {
    ...
    while (!feof(fp)) {
        char str[MAX_X];
        fscanf(fp, "%s\n", str);
        for (int j = 0; j < (int)sizeof(str); ++j)
            if (str[j] == 'X') universe[j][i] = true;
        ++i;
    }
}

void draw(bool universe[MAX_X][MAX_Y]) {
    clear();
    for (int x = 0; x < MAX_X; ++x)
        for (int y = 0; y < MAX_Y; ++y)
            if (universe[x][y]) mvaddch(y, x, ACS_BLOCK);
    refresh();
}
...
int main(int argc, char *argv[]) {
    ...
    for (int x = 0; x < MAX_X; ++x)
        for (int y = 0; y < MAX_Y; ++y) universe[x][y] = 0;
    ...
    while (true) {
        ...
        for (int x = 0; x < MAX_X; ++x)
            for (int y = 0; y < MAX_Y; ++y) newUniverse[x][y] = universe[x][y];
        ...
        for (int x = 0; x < MAX_X; ++x)
            for (int y = 0; y < MAX_Y; ++y) universe[x][y] = newUniverse[x][y];
